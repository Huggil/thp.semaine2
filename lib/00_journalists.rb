...
def count(journalists)
    puts "Il y a #{journalists.length} journalistes"
end 


def minimum(journalists)
    puts "#{journalists.sort_by(&:length)[0]}"
end


def size(journalists)
    puts "Il y a :
    #{journalists.count {|x| x.length == 6 }} journalistes qui ont 5 lettres"
end


def capital_letters(journalists)
    puts "nombre de peudos qui débuttent avec une majuscule"
    maj_letters = ('A'..'Z')
    puts journalists.count {|x| maj_letters.include?(x[1])}
end


def alpha(journalists)
    puts "voici l'ordre alphabétique"  
    puts journalists.sort_by { |x| x.downcase}
end


def num(journalists)
    puts "du plus petit au plus grand ça donne: "
    puts journalists = journalists.sort_by { |x| x.length }
end


def find(journalists)
    puts "@epenser est: "
    puts journalists.index("@epenser")
end


    def list(journalists)
    i = 0
    journalists.max { |a, b| a.length <=> b.length }
    while (i <= 33)
        print "nombre d'éléments avec #{i} caractères: "
